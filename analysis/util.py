...
# 定义关键词进行表格处理
BID_COMPANY_PATTERN = u"(?:中标|成交|投标|落标)(?:候选|)(?:单位|供应商|供货商|商|人)"
...
KEYWORDS = [u'项目名', u'项目名称', u'项目编号', u'项目联系人', u'采购项目', u'项目联系电话', u'项目经理', u'项目负责人',
            ...
            u'得分', u'排名', u'合同包', u'中标候选人', u'工商注册号', u'组织机构代码', u'中标结果', u'入围单位', u'名次', u'中标公示信息',
            u'接受投诉部门', u'业绩加分项明细', u'落标供应商', u'落标原因', u'范围', u'内容']
...
def type_of_text(text):
    ...
    if re.search(u"[:：]+", text) is not None:
        return "kv"
    # 关键字最大长度不超过 30
    t = re.sub(BID_COMPANY_PATTERN, u"", text)
    t = re.sub(BID_MONEY_PATTERN, u"", t)
    t = re.sub(u"|".join([k for k in KEYWORDS if k]), u"", t)
    if len(t) > 10 or len(text) > 30:
        return "v"
    # 是一个句子而不是一个短语
    if re.search(u"[，；。！？,;!?]+", text) is not None:
        return "v"
...
def split_text_by_strings(strings, text):
    ...
    for string in strings:
        ...
        if ind >= 0:
            ...
            result.append((prefix_text, suffix_text))
