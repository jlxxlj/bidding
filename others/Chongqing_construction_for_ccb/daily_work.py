...
def init_table():
    ...
    ed = datetime.datetime(year=2017, month=2, day=28)
    data = export_chongqing_construction(st, ed)
...
def prepare():
    ...
    dt = datetime.date(year=2017, month=3, day=1)
    ed = datetime.date(year=2017, month=3, day=19)
    while dt < ed:
        data = export_chongqing_construction(dt, dt)
...
def write_excel(path, column_head, data):
    ...
    sheet1 = f.add_sheet(sheetname=u"重庆地区工程建设中标信息", cell_overwrite_ok=True)
    # 写入列名
...
def export_chongqing_construction(begin, end):
    # sql = """
    #     select
    #         announce.region as "region",
    #         to_char(announce.published_ts, 'YYYY-MM-DD') as "published_ts",
...
def work_on_date(date):
    ...
    if len(dt) <= 0:
        ...
        data = export_chongqing_construction(pre_date, date)
    ...
    path = u"data/重庆%s工程建设项目中标结果.xls" % time_str
    path = os.path.join(folder_path, path)
    write_excel(path, column_head, data)

    for receiver in EMAIL_RECEIVERS:
        send_email(receiver,
                   subject=u"重庆地区%s工程建设中标信息" % time_str,
...
if __name__ == "__main__":
    # prepare()
    daily_work()
    work_on_date(datetime.date(2017, 3, 17))
