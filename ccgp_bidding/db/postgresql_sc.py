...
class PostgresqlUtil(object):
    ...
    def insert(self, table_name, dict_data):
        insert_sql = generate_sql_string("INSERT", table_name, dict_data)
        conn = self.get_conn()
        cur = conn.cursor()
    ...
    def delete(self, table_name, dict_condition):
        delete_sql = generate_sql_string("DELETE", table_name, dict_condition=dict_condition)
        conn = self.get_conn()
        cur = conn.cursor()
    ...
    def update(self, table_name, dict_data, dict_condition, upsert=False):
        item = self.select(table_name, dict_condition)
        if len(item) == 0 and upsert is False:
            raise ValueError()
        elif len(item) == 0 and upsert is True:
            self.insert(table_name, dict_data)
        else:  # len(item) > 0
            update_sql = generate_sql_string("UPDATE", table_name, dict_data, dict_condition)
            conn = self.get_conn()
            cur = conn.cursor()
    ...
    def select(self, table_name, dict_condition):
        select_sql = generate_sql_string("SELECT", table_name, dict_condition=dict_condition)
        conn = self.get_conn()
        cur = conn.cursor()
