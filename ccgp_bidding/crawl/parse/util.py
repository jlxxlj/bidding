...
KEYWORDS = [u'项目编号', u'项目联系人', u'采购项目', u'项目联系电话', u'项目经理', u'项目负责人',

            u'联系方式', u'小组成员', u'时间', u"日期", u'地点', u'地址', u'编号',  u'公告媒体', u'评标小组组长',

            u'采购人', u'采购单位', u'采购代理', u'采购方式',u'采购编号', u'采购单位', u'采购内容', u'采购结果',
            ...
            u'得分', u'排名', u'', u'', u'', u'', u'', u'', u'', u'', u'', u'', u'', u'']


def type_of_text(text):
    # 有三种类型：1.关键字 2.值 3.键值一起
    txts = re.split(u"\(.*[:：]+.*\)|（.*[:：]+.*）", text)
    for txt in txts:
        if re.search(u"[:：]+", txt) is not None:
            return "kv"
    if len(text) > 20:  # 关键字最大长度不超过20
        return "v"
...
def convert_cn_number(cn_number):
    ...
    for number in cn_number:
        if number in dict_cn_number:
            ...
            if v == 10 and len(tmp) == 0:  # 金额是"拾"开头的情况
                tmp.append([10., 10.])
            elif v >= 1e1:  # 金额单位
                t = 0
